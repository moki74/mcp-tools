  ...
  async bulkUpdate(params: {
    ...
    if (!validateBulkUpdate(params)) {
      return {
        status: "error",
        error:
          "Invalid parameters: " + JSON.stringify(validateBulkUpdate.errors),
      };
    }
        ...
        try {
          ...
          for (const update of batch) {
            ...
            const setClause = Object.entries(update.data)
              .map(
                ([column, _]) =>
                  `${this.security.escapeIdentifier(column)} = ?`,
              )
              .join(", ");
            ...
            update.conditions.forEach((condition) => {
              const escapedField = this.security.escapeIdentifier(
                condition.field,
              );
              switch (condition.operator) {
                case "eq":
  ...
  async bulkDelete(params: {
    ...
    if (!validateBulkDelete(params)) {
      return {
        status: "error",
        error:
          "Invalid parameters: " + JSON.stringify(validateBulkDelete.errors),
      };
    }
        ...
        try {
          ...
          for (const conditions of batch) {
            ...
            const paramValidation =
              this.security.validateParameters(whereValues);
            if (!paramValidation.valid) {
              throw new Error(`Invalid parameters: ${paramValidation.error}`);
            }
