...
export class SchemaDesignTools {
  ...
  async designSchemaFromRequirements(params: {
    ...
    entities?: Array<{ name: string; fields?: string[] }>; // Optional explicit hints
    ...
    try {
      ...
      for (const e of explicitEntities) entityNames.add(e.name.trim());
      for (const e of extracted.entities) entityNames.add(e);
      if (entityNames.size === 0) {
        // Fallback: create a single generic table
        entityNames.add("items");
        notes.push(
          "No entities could be inferred from the text; generated a single fallback table 'items'.",
        );
      }
      ...
      const rawNameToTable = new Map<string, string>();
      ...
      for (const t of tablesSpec) {
        const explicit = explicitEntities.find(
          (e) => this.normalizeLoose(e.name) === this.normalizeLoose(t._raw_name),
        );
      ...
      for (const t of tablesSpec) {
        if (!this.security.validateIdentifier(t.table_name).valid) {
          notes.push(
            `Generated table name '${t.table_name}' may be invalid. Consider renaming.`,
          );
        }
        for (const c of t.columns) {
          if (!this.security.validateIdentifier(c.name).valid) {
            notes.push(
              `Generated column name '${t.table_name}.${c.name}' may be invalid. Consider renaming.`,
            );
          }
        }
      }
  ...
  private inferColumnType(colName: string): {
    column: { name: string; type: string; nullable: boolean; unique?: boolean };
    uniqueIndex?: boolean;
  } {
  ...
  private extractEntitiesAndRelations(text: string): {
    entities: string[];
    relationships: Array<{ from: string; to: string; type: "one_to_many" | "many_to_one" }>;
    fieldsByEntity: Map<string, string[]>;
  } {
    const entities = new Set<string>();
    const relationships: Array<{ from: string; to: string; type: "one_to_many" | "many_to_one" }> = [];
    ...
    for (const match of raw.matchAll(
      /\b([a-zA-Z][a-zA-Z0-9_-]{1,40})\s+(?:fields|columns|attributes)\s*:\s*([^\n\.]+)(?:\.|\n|$)/gi,
    )) {
  ...
  private findTableNameForRaw(
    raw: string,
    tablesSpec: Array<{ table_name: string; _raw_name: string }>,
  ): string | undefined {
  ...
  private makeIndexName(table: string, columns: string[], unique: boolean): string {
