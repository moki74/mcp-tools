...
export class StoredProcedureTools {
   ...
   */
  async listStoredProcedures(params: { database?: string }): Promise<{
    status: string;
    data?: any[];
    error?: string;
  }> {
    try {
      ...
      if (!validateListStoredProcedures(params)) {
        return {
          ...
          error:
            "Invalid parameters: " +
            JSON.stringify(validateListStoredProcedures.errors),
        };
      }
    ...
    try {
      ...
      if (!validateGetStoredProcedureInfo(params)) {
        return {
          ...
          error:
            "Invalid parameters: " +
            JSON.stringify(validateGetStoredProcedureInfo.errors),
        };
      }
  ...
  async executeStoredProcedure(params: {
    ...
    if (!validateStoredProcedureExecution(params)) {
      return {
        ...
        error:
          "Invalid parameters: " +
          JSON.stringify(validateStoredProcedureExecution.errors),
      };
    }
    ...
    try {
      ...
      const identifierValidation =
        this.security.validateIdentifier(procedure_name);
      ...
      const callResults = await this.db.query<any>(
        callQuery,
        paramValidation.sanitizedParams!.slice(0, paramIndex),
      );
      ...
      return {
        ...
        data: {
          results: callResults,
          outputParameters:
            Object.keys(outputValues).length > 0 ? outputValues : undefined,
        },
      };
    } catch (error: any) {
      return {
  ...
  async createStoredProcedure(params: {
    ...
    if (!validateStoredProcedureCreation(params)) {
      return {
        ...
        error:
          "Invalid parameters: " +
          JSON.stringify(validateStoredProcedureCreation.errors),
      };
    }
    ...
    try {
      ...
      if (!validateDropStoredProcedure(params)) {
        return {
          ...
          error:
            "Invalid parameters: " +
            JSON.stringify(validateDropStoredProcedure.errors),
        };
      }
    ...
    try {
      ...
      if (!validateShowCreateProcedure(params)) {
        return {
          ...
          error:
            "Invalid parameters: " +
            JSON.stringify(validateShowCreateProcedure.errors),
        };
      }
