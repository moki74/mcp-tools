...
export class ForecastingTools {
  ...
  private extractExplainNodes(explainJson: any): ExplainTableNode[] {
    const nodes: ExplainTableNode[] = [];
    const visit = (obj: any) => {
  ...
  async predictQueryPerformance(params: {
    ...
    try {
      ...
      const explainRows = await this.db.query<any[]>(
        `EXPLAIN FORMAT=JSON ${query}`,
      );

      let explainJson: any = null;
      ...
      const tablePredictions = nodes
        ...
        .map((n) => {
          const t = n.table_name as string;
          const factor =
            typeof perTable[t] === "number" && perTable[t] > 0
              ? perTable[t]
              : growth;
          const baseRows = n.rows_examined_per_scan ?? n.rows_produced_per_join ?? null;
          const predictedRows =
            typeof baseRows === "number" ? Math.round(baseRows * factor) : null;
          return {
            table_name: t,
      ...
      const avgFactor =
        tablePredictions.length > 0
          ? tablePredictions.reduce((acc, t) => acc + t.growth_factor, 0) /
            tablePredictions.length
          : growth;

      const predictedCost =
        typeof queryCost === "number" && Number.isFinite(queryCost)
          ? parseFloat((queryCost * avgFactor).toFixed(4))
          : null;

      const worstScan = tablePredictions.reduce(
        (max, t) => Math.max(max, t.predicted_rows_estimate || 0),
        0,
      );

      let risk: "low" | "medium" | "high" = "low";
      if (worstScan > 1_000_000) risk = "high";
      else if (worstScan > 100_000) risk = "medium";

      const recommendations: string[] = [];
      for (const t of tablePredictions) {
        if ((t.access_type || "").toUpperCase() === "ALL") {
          recommendations.push(
            `Table '${t.table_name}' is using a full scan (access_type=ALL). Consider adding an index aligned with WHERE/JOIN predicates.`,
          );
        }
        if (!t.key && (t.predicted_rows_estimate || 0) > 100_000) {
          recommendations.push(
            `Table '${t.table_name}' has no chosen index in EXPLAIN and predicted scan is large; review indexing and query predicates.`,
          );
        }
      }
  ...
  async forecastDatabaseGrowth(params: {
    ...
    try {
      ...
      let baseDailyRate: number | null = null;
      ...
      if (baseDailyRate === null && Object.keys(perTableRates).length === 0) {
        return {
          status: "error",
          error:
            "Provide growth_rate_percent_per_day or growth_rate_percent_per_month, or per_table_growth_rate_percent_per_day.",
        };
      }

      const rows = await this.db.query<TableSizeRow[]>(
        `
      ...
      const totals = {
        current_total_bytes: tableForecasts.reduce(
          (acc, t) => acc + t.current.total_size_bytes,
          0,
        ),
        forecast_total_bytes: tableForecasts.reduce(
          (acc, t) => acc + t.forecast.total_size_bytes,
          0,
        ),
      };
      ...
      return {
        ...
        data: {
          database,
          horizon_days: horizonDays,
          base_growth_rate_percent_per_day:
            baseDailyRate === null ? null : baseDailyRate * 100,
