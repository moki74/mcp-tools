...
export class MigrationTools {
  ...
  async copyTableData(params: {
    ...
    try {
      ...
      const sourceColumns: any[] = await this.db.query(sourceColumnsQuery);
      queryCount++;
      ...
      const countResult: any[] = await this.db.query(countQuery);
      queryCount++;
      ...
      while (offset < totalRows) {
        ...
        const rows: any[] = await this.db.query(selectQuery);
        queryCount++;
  ...
  async compareTableStructure(params: {
    ...
    try {
      ...
      const onlyInTable1: string[] = [];
      const onlyInTable2: string[] = [];
      const different: Array<{ column: string; table1: any; table2: any }> = [];
