...
export class TestDataTools {
  ...
  private clampString(value: string, maxLen: number | null): string {
    if (!maxLen || maxLen <= 0) return value;
  ...
  private generateValueForColumn(
    ...
    if (dataType === "enum") {
      ...
      if (values.length > 0) return values[rowIndex % values.length];
    ...
    if (["date", "datetime", "timestamp"].includes(dataType)) {
      ...
      if (dataType === "date") return dt.toISOString().slice(0, 10);
      return dt;
    }
    ...
    if (["varchar", "char", "text", "tinytext", "mediumtext", "longtext"].includes(dataType)) {
      let v = "";
      if (colNameLower.includes("email")) v = `user${rowIndex + 1}@example.com`;
      else if (colNameLower.includes("phone")) v = `+1555000${String(rowIndex + 1).padStart(4, "0")}`;
      else if (colNameLower.includes("url")) v = `https://example.com/item/${rowIndex + 1}`;
      else if (colNameLower.includes("name")) v = `Sample ${col.COLUMN_NAME} ${rowIndex + 1}`;
      else if (colNameLower.includes("title")) v = `Title ${rowIndex + 1}`;
      else if (colNameLower.includes("description")) v = `Description for row ${rowIndex + 1}`;
      else v = `${col.COLUMN_NAME}_${rowIndex + 1}`;
  ...
  async generateTestData(params: {
    ...
    try {
      ...
      const fks = await this.db.query<ForeignKeyInfo[]>(
        `
          SELECT
            COLUMN_NAME,
      ...
      const fkSamplesByColumn = new Map<string, any[]>();
      ...
      for (const fk of fks) {
        ...
        if (!this.security.validateIdentifier(refTable).valid) continue;
        if (!this.security.validateIdentifier(refCol).valid) continue;

        const escapedRefTable = this.security.escapeIdentifier(refTable);
        const escapedRefCol = this.security.escapeIdentifier(refCol);

        const sampleRows = await this.db.query<any[]>(
          `SELECT ${escapedRefCol} as v FROM ${escapedRefTable} WHERE ${escapedRefCol} IS NOT NULL LIMIT 200`,
        );
        const sampleValues = sampleRows.map((r) => r.v).filter((v) => v !== null);
        if (sampleValues.length === 0) {
          warnings.push(
            `Foreign key '${table_name}.${fk.COLUMN_NAME}' references '${refTable}.${refCol}' but no referenced key samples were found (referenced table may be empty).`,
          );
        }
      ...
      const insertColumns = columns.filter((c) => {
        ...
        if (extra.includes("generated")) return false;
        return true;
      });
      ...
      const escapedCols = insertColumns.map((c) =>
        this.security.escapeIdentifier(c.COLUMN_NAME),
      );
      ...
      for (let start = 0; start < row_count; start += batchSize) {
        ...
        for (let i = start; i < end; i++) {
          ...
          for (const col of insertColumns) {
            ...
            let v: any = this.generateValueForColumn(col, i, fkSamples);

            // Null handling
            if (!includeNulls && col.IS_NULLABLE === "YES") {
              // avoid NULLs unless needed
              if (v === null) v = this.generateValueForColumn(col, i, undefined);
            }
          ...
          if (previewRows.length < 20) previewRows.push(rowObj);
