 ...
 */
export class DocumentationGeneratorTools {
    ...
    async generateDocumentation(params: {
        ...
        try {
         ...
         ORDER BY TABLE_NAME, INDEX_NAME, SEQ_IN_INDEX`,
            ...
            let businessGlossary: Map<string, string> = new Map();
            ...
            let tableStats: Map<string, any> = new Map();
    ...
    async generateDataDictionary(params: {
        ...
        try {
            ...
            // Get table info
            const tableInfo = await this.db.query<any[]>(
         ...
         WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND REFERENCED_TABLE_NAME IS NOT NULL`,
            ...
            // Get indexes
            const indexResult = await this.db.query<any[]>(
         ...
         ORDER BY INDEX_NAME, SEQ_IN_INDEX`,
            ...
            if (include_constraints) {
                try {
                    checkConstraints = await this.db.query<any[]>(
            ...
            const columns = await Promise.all(
                columnsResult.map(async (col) => {
                    ...
                    if (fk) constraints.push(`FOREIGN KEY -> ${fk.REFERENCED_TABLE_NAME}`);

                    // Get sample values
                    let sampleValues: any[] | undefined;
            ...
            const indexMap = new Map<string, { columns: string[]; is_unique: boolean }>();
        ...
        try {
         ...
         ORDER BY TABLE_NAME, ORDINAL_POSITION`,
            ...
            for (const entry of glossary) {
                ...
                if (related.length > 1) {
                    entry.related_terms = related.filter(
                        (r) => r !== `${entry.source_table}.${entry.technical_name}`
                    );
                }
            }
     ...
     */
    private generateBusinessGlossary(columns: any[]): Map<string, string> {
    ...
    private async getTableStatistics(
        ...
        const result = await this.db.query<any[]>(
            `SELECT COUNT(*) as row_count FROM \`${database}\`.\`${tableName}\``
        );
        return {
    ...
    private generateMarkdown(
        ...
        if (foreignKeys.length > 0) {
            ...
            for (const fk of foreignKeys) {
                lines.push(
                    `    ${fk.TABLE_NAME} ||--o{ ${fk.REFERENCED_TABLE_NAME} : "${fk.COLUMN_NAME}"`
                );
            }
            lines.push("```");
            lines.push("");
        ...
        for (const table of tables) {
            ...
            if (stats) {
                lines.push(
                    `**Rows:** ${stats.row_count} | **Engine:** ${table.ENGINE || "N/A"}`
                );
                lines.push("");
            }
            ...
            for (const col of tableCols) {
                ...
                lines.push(
                    `| \`${col.COLUMN_NAME}\` | ${col.COLUMN_TYPE} | ${col.IS_NULLABLE} | ${keyBadge} | ${description} |`
                );
            }
            lines.push("");
            ...
            if (tableIndexes.length > 0) {
                ...
                for (const [indexName, cols] of indexGroups) {
                    const isUnique = tableIndexes.find(
                        (i) => i.INDEX_NAME === indexName
                    )?.NON_UNIQUE === 0;
                    lines.push(
                        `- **${indexName}**${isUnique ? " (UNIQUE)" : ""}: ${cols.join(", ")}`
                    );
                }
                lines.push("");
            }
            ...
            if (includeExamples) {
                ...
                lines.push(
                    `SELECT * FROM \`${database}\`.\`${table.TABLE_NAME}\` LIMIT 10;`
                );
                lines.push("");
                lines.push(`-- Count records`);
                lines.push(
                    `SELECT COUNT(*) FROM \`${database}\`.\`${table.TABLE_NAME}\`;`
                );
                lines.push("```");
                lines.push("");
            }
        ...
        if (businessGlossary.size > 0) {
            ...
            const sortedTerms = Array.from(businessGlossary.entries()).sort((a, b) =>
    ...
    private generateHTML(
        ...
        for (const table of tables) {
            html.push(
                `      <li><a href="#${table.TABLE_NAME}">${table.TABLE_NAME}</a></li>`
            );
        }
        html.push("    </ul>");
        html.push("  </div>");
        ...
        for (const table of tables) {
            ...
            if (stats) {
                html.push(
                    `  <p><strong>Rows:</strong> ${stats.row_count} | <strong>Engine:</strong> ${table.ENGINE || "N/A"}</p>`
                );
            }
            ...
            for (const col of tableCols) {
                ...
                html.push(
                    `    <tr><td><code>${col.COLUMN_NAME}</code></td><td>${col.COLUMN_TYPE}</td><td>${col.IS_NULLABLE}</td><td>${keyBadge}</td><td>${description}</td></tr>`
                );
            }
            html.push("  </table>");
     ...
     */
    private groupIndexes(indexes: any[]): any[] {
        const groups = new Map<string, { columns: string[]; is_unique: boolean }>();
     ...
     */
    private inferBusinessTerm(columnName: string): string {
     ...
     */
    private inferColumnDescription(columnName: string, dataType: string): string {
    ...
    private inferTableDescription(tableName: string): string {
        ...
        if (name.includes("order")) return "Stores order records";
        if (name.includes("product")) return "Stores product catalog";
        if (name.includes("customer")) return "Stores customer information";
        if (name.includes("invoice")) return "Stores invoice records";
        if (name.includes("payment")) return "Stores payment transactions";
        if (name.includes("log")) return "Stores activity/event logs";
        if (name.includes("config") || name.includes("setting")) return "Stores configuration settings";
        if (name.includes("session")) return "Stores session data";
        if (name.includes("cache")) return "Stores cached data";
        if (name.includes("migration")) return "Stores database migration history";
     ...
     */
    private inferCategory(columnName: string, dataType: string): string {
        const name = columnName.toLowerCase();

        if (name === "id" || name.endsWith("_id")) return "Identifiers";
        ...
        if (name.includes("email") || name.includes("phone") || name.includes("address"))
            return "Contact";
        if (name.includes("name") || name.includes("title")) return "Names";
        ...
        if (name.startsWith("is_") || name.startsWith("has_")) return "Flags";
