...
export class QueryVisualizationTools {
  ...
  private sqlJoinEdges(query: string): Array<{ from: string; to: string; on?: string }> {
    const edges: Array<{ from: string; to: string; on?: string }> = [];
  ...
  private buildMermaid(nodes: ExplainTableNode[], edges: Array<{ from: string; to: string; on?: string }>): string {
    const lines: string[] = [];
    lines.push("graph TD");

    const uniqTables = Array.from(
      new Set(nodes.map((n) => n.table_name).filter(Boolean) as string[]),
    );

    const nodeId = (t: string) => `t_${t.replace(/[^a-zA-Z0-9_]/g, "_")}`;

    for (const t of uniqTables) {
      const n = nodes.find((x) => x.table_name === t);
      const labelParts: string[] = [t];
      if (n?.access_type) labelParts.push(`access=${n.access_type}`);
      if (n?.key) labelParts.push(`key=${n.key}`);
    ...
    if (edges.length) {
      for (const e of edges) {
        lines.push(
          `${nodeId(e.from)} -->|"${(e.on || "").replace(/"/g, "'").slice(0, 60)}"| ${nodeId(e.to)}`,
        );
      }
      return lines.join("\n");
    }
  ...
  async visualizeQuery(params: {
    ...
    try {
      ...
      const explainRows = await this.db.query<any[]>(explainQuery);

      let explainJson: any = null;
      let queryCost: string | null = null;
      ...
      if (includeExplain) data.explain_json = explainJson;
