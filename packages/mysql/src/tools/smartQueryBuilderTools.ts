...
const activeSessions = new Map<string, QueryBuilderSession>();

export class SmartQueryBuilderTools {
    ...
    private async getDatabaseSchema(database: string): Promise<{
        tables: Array<{
            name: string;
            columns: Array<{
                name: string;
    ...
    private findQueryTemplate(intent: string): QueryTemplate | null {
    ...
    private suggestNextTables(currentTables: string[], availableTables: string[], intent: string): string[] {
        const template = this.findQueryTemplate(intent);
        if (template && template.template_state.tables) {
            return template.template_state.tables.filter(table => 
                !currentTables.includes(table) && availableTables.includes(table)
            );
        }
    ...
    private generateQueryFromState(state: QueryBuildingState): string {
        let query = "SELECT ";

        if (state.selected_columns.length === 0) {
            query += "*";
        } else {
            const columnList = state.selected_columns.map(col => 
                col.alias ? `${col.table}.${col.column} AS ${col.alias}` : `${col.table}.${col.column}`
            ).join(", ");
            query += columnList;
        }
        ...
        if (state.aggregations.length > 0 && state.group_by.length === 0) {
            query = "SELECT " + state.aggregations.map(agg => 
                `${agg.function}(${agg.table}.${agg.column}) AS ${agg.alias}`
            ).join(", ") + ` FROM ${state.tables[0]}`;
        }
    ...
    async startQueryBuilder(params: {
        ...
        try {
            ...
            const session: QueryBuilderSession = {
    ...
    async addTablesToQuery(params: {
        ...
        try {
            ...
            const validTables = tables.filter(table => 
                session.available_tables.some(t => t.name === table)
            );
    ...
    async defineJoins(params: {
        ...
        try {
            ...
            const validJoins = joins.filter(join => 
                session.state.tables.includes(join.from_table) && 
    ...
    async executeQuery(params: {
        ...
        try {
            ...
            const results = await this.db.query<any[]>(generatedQuery);
    ...
    async getQueryTemplates(params: {
        ...
        try {
            ...
            if (category) {
                filteredTemplates = queryTemplates.filter(template => 
                    template.template_state.context === category
                );
            }
    ...
    async suggestNextStep(params: {
        ...
        try {
            ...
            switch (session.current_step) {
                case "select_tables":
                    const suggestedTables = this.suggestNextTables(
                        session.state.tables,
                        session.available_tables.map(t => t.name),
