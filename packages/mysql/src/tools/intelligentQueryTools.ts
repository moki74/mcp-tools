 ...
 */
export class IntelligentQueryTools {
    ...
    async buildQueryFromIntent(params: {
        ...
        try {
            ...
            const matchedEntities = this.matchEntitiesToSchema(
                intentAnalysis,
                schemaContext
            );

            // Step 4: Generate SQL based on analysis
            const generatedQuery = this.generateSQL(
                intentAnalysis,
                matchedEntities,
                context,
                max_complexity,
     ...
     */
    private async getSchemaContext(database: string): Promise<{
        tables: Array<{
            name: string;
            columns: Array<{
                name: string;
    ...
    private analyzeIntent(
        ...
        action: "select" | "count" | "aggregate" | "join" | "unknown";
        keywords: string[];
        aggregations: string[];
        conditions: string[];
        orderBy: string | null;
        ...
        // Detect action type
        let action: "select" | "count" | "aggregate" | "join" | "unknown" = "unknown";
        ...
        const keywords = words.filter(
            (w) => !this.isStopWord(w) && /^[a-z_]+$/i.test(w)
        );

        // Detect aggregation functions
        const aggregations: string[] = [];
        if (/\btotal\b|\bsum\b/i.test(text)) aggregations.push("SUM");
        if (/\baverage\b|\bavg\b/i.test(text)) aggregations.push("AVG");
        if (/\bmin(imum)?\b/i.test(text)) aggregations.push("MIN");
        if (/\bmax(imum)?\b/i.test(text)) aggregations.push("MAX");
        if (/\bcount\b|\bhow many\b/i.test(text)) aggregations.push("COUNT");
    ...
    private matchEntitiesToSchema(
        ...
        for (const table of schemaContext.tables) {
            ...
            for (const keyword of intent.keywords) {
                ...
                // Partial match
                    ...
                    this.similarityScore(tableName, kw) > 0.6
                ) {
                    tableScore += 3;
                }
            }
        ...
        if (matchedTables.length === 0 && schemaContext.tables.length > 0) {
            const largestTable = schemaContext.tables.reduce((a, b) =>
        ...
        if (matchedTables.length > 1) {
            for (const rel of schemaContext.relationships) {
                if (
                    matchedTables.includes(rel.fromTable) &&
        ...
        const primaryTable =
            matchedTables.length > 0
                ? matchedTables.reduce((a, b) =>
                    (tableScores.get(a) || 0) > (tableScores.get(b) || 0) ? a : b
                )
                : null;
    ...
    private generateSQL(
        intent: ReturnType<typeof this.analyzeIntent>,
        matchedEntities: ReturnType<typeof this.matchEntitiesToSchema>,
        context: string,
        ...
        const columns =
            matchedEntities.tableToColumns.get(primaryTable) || [];
        ...
        if (intent.orderBy) {
            const direction = /\b(latest|newest|highest|last)\b/i.test(intent.orderBy)
                ...
                : "ASC";
            const orderCol =
                intent.orderBy.match(/^(latest|newest|oldest|highest|lowest|first|last)$/i)
                    ? columns[0] || "id"
                    : intent.orderBy;
        ...
        if (!groupByClause && columns.length > 0) {
            alternatives.push(
                `SELECT ${columns[0]}, COUNT(*) AS count FROM \`${database}\`.\`${primaryTable}\`${whereClause} GROUP BY \`${columns[0]}\` ORDER BY count DESC LIMIT 10`
            );
        }
     ...
     */
    private estimateComplexity(sql: string): string {
        const lowerSql = sql.toLowerCase();
        let score = 0;

        if (lowerSql.includes("join")) score += 2;
        if (lowerSql.includes("group by")) score += 1;
        if (lowerSql.includes("having")) score += 1;
        if (lowerSql.includes("subquery") || (lowerSql.match(/select/g) || []).length > 1)
            score += 3;
        if (lowerSql.includes("union")) score += 2;
        if ((lowerSql.match(/and|or/g) || []).length > 3) score += 1;
    ...
    private generateOptimizationHints(
        ...
        const largeTables = schemaContext.tables.filter(
            (t) => matchedEntities.tables.includes(t.name) && t.columns.length > 20
        );
        if (largeTables.length > 0) {
            hints.push(
                `Tables ${largeTables.map((t) => t.name).join(", ")} have many columns. Select only needed columns.`
            );
        }
    ...
    async suggestQueryImprovements(params: {
        ...
        try {
            ...
            if (
                (lowerQuery.includes("delete") || lowerQuery.includes("update")) &&
                !lowerQuery.includes("where")
            ) {
                suggestions.push({
                    type: "SAFETY",
                    description:
                        "DELETE/UPDATE without WHERE clause will affect all rows - add conditions",
                });
            }
            ...
            if (lowerQuery.match(/like\s+['"]%/)) {
                suggestions.push({
                    type: "INDEX_USAGE",
                    description:
                        "Leading wildcard in LIKE pattern prevents index usage. Consider FULLTEXT search.",
                });
            }
            ...
            if (lowerQuery.match(/where\s+\w+\s*\([^)]+\)\s*=/)) {
                suggestions.push({
                    type: "INDEX_USAGE",
                    description:
                        "Using functions on columns in WHERE clause prevents index usage. Move function to the right side.",
                });
            }
            ...
            if (lowerQuery.match(/select\s+distinct.*,.*,/)) {
                suggestions.push({
                    type: "PERFORMANCE",
                    description:
                        "SELECT DISTINCT on multiple columns can be slow. Consider using GROUP BY instead.",
                });
            }
            ...
            if (optimization_goal === "memory") {
                ...
                if (lowerQuery.includes("order by")) {
                    suggestions.push({
                        type: "MEMORY",
                        description:
                            "ORDER BY requires memory for sorting. Consider indexing the ORDER BY column.",
                    });
                }
            }
            ...
            if (optimization_goal === "readability") {
                if (!lowerQuery.includes("\n")) {
                    suggestions.push({
                        type: "READABILITY",
                        description:
                            "Consider formatting query with line breaks for better readability.",
                    });
                }
            ...
            return {
                ...
                data: {
                    ...
                    estimated_improvement:
                        suggestions.length > 2 ? "SIGNIFICANT" : suggestions.length > 0 ? "MODERATE" : "MINIMAL",
                },
            };
        } catch (error: any) {
            return {
