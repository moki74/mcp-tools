...
export class ExplainAnalyzer {
    private static instance: ExplainAnalyzer;

    private constructor() { }

    public static getInstance(): ExplainAnalyzer {
     ...
     */
    public analyze(explainResult: any[]): ExplainAnalysis {
        const issues: string[] = [];
        ...
        for (const row of explainResult) {
            ...
            if (row.type === 'ALL') maxComplexity = Math.max(maxComplexity, 3);
            else if (row.type === 'index') maxComplexity = Math.max(maxComplexity, 2);
            else maxComplexity = Math.max(maxComplexity, 1);
        }

        let complexity: "LOW" | "MEDIUM" | "HIGH" = "LOW";
        if (maxComplexity >= 3) complexity = "HIGH";
        else if (maxComplexity === 2) complexity = "MEDIUM";
    ...
    private analyzeJsonFormat(jsonString: string): ExplainAnalysis {
        try {
    ...
    private traverseJsonPlan(node: any, issues: string[], suggestions: string[]) {
        if (!node) return;
    ...
    private analyzeRow(row: any, issues: string[], suggestions: string[]) {
    ...
    private generateSummary(complexity: string, issues: string[]): string {
        if (issues.length === 0) {
