...
exports.validateReadRecords = validateReadRecords;
exports.validateUpdateRecord = validateUpdateRecord;
exports.validateDeleteRecord = validateDeleteRecord;
exports.validateQuery = validateQuery;
exports.validateBulkInsert = validateBulkInsert;
...
exports.validateImportFromJSON = validateImportFromJSON;
...
function validateCreateRecord(data) {
...
function validateReadRecords(data) {
...
function validateUpdateRecord(data) {
...
function validateDeleteRecord(data) {
...
function validateQuery(data) {
    ...
    if (data && data.params && Array.isArray(data.params)) {
        if (data.params.length > exports.INPUT_LIMITS.MAX_PARAMETERS) {
            return {
                valid: false,
                errors: [`Parameters array exceeds maximum length of ${exports.INPUT_LIMITS.MAX_PARAMETERS} items`]
            };
        }
...
function validateBulkInsert(data) {
...
function formatErrors(errors) {
    return errors.map(error => {
        ...
        const errorMessage = error.schema?.errorMessage || message;
...
function validateValue(value) {
    ...
    if (typeof value === 'string') {
        if (value.length > exports.INPUT_LIMITS.MAX_STRING_LENGTH) {
            return { valid: false, error: `String value exceeds maximum length of ${exports.INPUT_LIMITS.MAX_STRING_LENGTH} characters` };
        }
...
function validateImportFromJSON(data) {
    try {
        const valid = ajv.validate(importFromJsonSchema, data);
        if (!valid) {
            const errors = ajv.errors?.map((error) => error.message || 'Validation error') || ['Unknown validation error'];
        ...
        if (data.json_data) {
            ...
            try {
                ...
                if (data.json_data.length > exports.INPUT_LIMITS.MAX_STRING_LENGTH) {
                    return { valid: false, errors: [`JSON data exceeds maximum length of ${exports.INPUT_LIMITS.MAX_STRING_LENGTH} characters`] };
                }
            }
            catch (e) {
