    ...
    try {
        ...
        switch (name) {
            ...
            case "create_fulltext_index":
                result = await mysqlMCP.createFulltextIndex((args || {}));
                break;
            case "fulltext_search":
                result = await mysqlMCP.fulltextSearch((args || {}));
                break;
            case "get_fulltext_info":
                result = await mysqlMCP.getFulltextInfo((args || {}));
                break;
            case "drop_fulltext_index":
                result = await mysqlMCP.dropFulltextIndex((args || {}));
                break;
            case "get_fulltext_stats":
                result = await mysqlMCP.getFulltextStats((args || {}));
                break;
            case "optimize_fulltext":
                result = await mysqlMCP.optimizeFulltext((args || {}));
                break;
            // ==========================================
            // PHASE 2: AI Enhancement Tools (Schema + Security + Indexing)
            // ==========================================
